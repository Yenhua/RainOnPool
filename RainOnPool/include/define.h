...
#define GRAVITY  3.0f//to modify the velocity after each step
...
struct Rain{
	...
	float weight;
	float velocity;//may be constant for now
	//technically velocity is a vector, but right now it's probably just speed in the y position.
	//should be -y, since we're moving in a downwards position
};
...
struct G308_Triangle {
	...
	unsigned int n2;
	unsigned int n3;
	unsigned int t1;//texture coordinates
	unsigned int t2;
	unsigned int t3;
};
